...
import {
  CreditCard, ShoppingCart, TrendingUp, BadgePercent,
  PieChart as PieChartIcon, Trophy, Calendar, ShoppingBag, Package, ExternalLink,
} from "lucide-react";
...
export default function Overview({ data }) {
  ...
  const {
    totalSpend, totalOrders, avgBasket, totalSaved,
    monthlySpend, ordersByDay,
    categorySpend, topProducts, topByFrequency,
    topOrders, inflationProducts, promoDependency, topBrands,
  } = data;
  ...
  const topConfig = { [topMode === "spend" ? "spend" : "count"]: { label: topMode === "spend" ? "Разход" : "Брой", color: "var(--brand)" } };

  const dayConfig = { count: { label: "Поръчки", color: "var(--brand)" } };
  const promoConfig = Object.fromEntries(
    promoDependency.map((c) => [c.category, { label: c.category, color: categoryColor(c.category) }])
  );
  ...
  return (
    <main className="mx-auto max-w-6xl space-y-8 px-6 py-8">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <StatTile label="Общо похарчено" value={`${fmt(totalSpend)} €`} color="var(--brand)" icon={CreditCard} />
        <StatTile label="Поръчки" value={totalOrders} color="#6366f1" icon={ShoppingCart} />
        <StatTile label="Средна стойност на поръчка" value={`${fmt(avgBasket)} €`} color="#f59e0b" icon={TrendingUp} />
        <StatTile label="Общо спестено" value={`${fmt(totalSaved)} €`} sub="от промоции и отстъпки" color="#10b981" icon={BadgePercent} />
      </div>
        ...
        <CardContent>
          <ChartContainer config={spendConfig} className="h-60 w-full">
            <AreaChart data={monthlySpend}>
              ...
              <Area type="monotone" dataKey="spend" stroke="var(--brand)" strokeWidth={2} fill="url(#spendGrad)" />
            </AreaChart>
          </ChartContainer>
        </CardContent>
      </Card>
            ...
            <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1.5">
              {categorySpend.slice(0, 12).map((c) => (
                <div key={c.category} className="flex items-center gap-2 text-xs text-muted-foreground">
                  <span className="size-2 shrink-0 rounded-full" style={{ background: categoryColor(c.category) }} />
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <div className="flex rounded-lg border p-0.5 text-xs">
              {[["spend", "Разход"], ["frequency", "Честота"]].map(([m, label]) => (
                <button
                  key={m}
          ...
          <CardContent>
            <ChartContainer config={topConfig} style={{ height: topList.length * 24 + 40 }} className="w-full">
              <BarChart data={topList} layout="vertical" margin={{ left: 0, right: 16 }}>
                <CartesianGrid horizontal={false} />
                <XAxis type="number" tickLine={false} axisLine={false} tickMargin={8} unit={topMode === "spend" ? " €" : ""} />
                <YAxis dataKey="name" type="category" width={160} tickLine={false} axisLine={false} tickMargin={8} tick={{ fontSize: 10 }} tickFormatter={(v) => v.length > 22 ? v.slice(0, 22) + "…" : v} />
                <ChartTooltip content={<ChartTooltipContent formatter={(v) => [topMode === "spend" ? `${fmt(v)} €` : `${v} пъти`]} />} />
                <Bar dataKey={topMode === "spend" ? "spend" : "count"} radius={[0, 4, 4, 0]} barSize={14}>
                  {topList.map((p) => (
          ...
          <CardContent>
            <ChartContainer config={dayConfig} className="h-52 w-full">
              <BarChart data={ordersByDay} margin={{ left: 0, right: 8 }}>
                ...
                <ChartTooltip content={<ChartTooltipContent formatter={(v) => [`${v} поръчки`]} />} />
                ...
                <li key={o.id} className="flex items-center justify-between gap-4 py-2.5 text-sm">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <a href={`https://www.ebag.bg/orders/${o.id}`} target="_blank" rel="noopener noreferrer" className="ml-auto shrink-0 inline-flex items-center text-muted-foreground hover:text-foreground transition-colors">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent>
            <ChartContainer config={promoConfig} style={{ height: promoDependency.length * 24 + 40 }} className="w-full">
              <BarChart data={promoDependency} layout="vertical" margin={{ left: 0, right: 16 }}>
                ...
                <YAxis dataKey="category" type="category" width={180} tickLine={false} axisLine={false} tickMargin={8} tick={{ fontSize: 10 }} />
                <ChartTooltip
                  content={({ active, payload }) => {
                    if (!active || !payload?.length) return null;
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <div className="flex rounded-lg border p-0.5 text-xs">
              {[["spend", "Разход"], ["count", "Честота"]].map(([m, label]) => (
                <button
                  key={m}
          ...
          <CardContent>
            <ChartContainer config={brandsConfig} style={{ height: topBrands.length * 24 + 40 }} className="w-full">
              <BarChart data={topBrands} layout="vertical" margin={{ left: 0, right: 16 }}>
                <CartesianGrid horizontal={false} />
                <XAxis type="number" tickLine={false} axisLine={false} tickMargin={8} unit={brandsMode === "spend" ? " €" : ""} />
                <YAxis dataKey="brand" type="category" width={160} tickLine={false} axisLine={false} tickMargin={8} tick={{ fontSize: 10 }} tickFormatter={(v) => v.length > 22 ? v.slice(0, 22) + "…" : v} />
                <ChartTooltip content={<ChartTooltipContent formatter={(v) => [brandsMode === "spend" ? `${fmt(v)} €` : `${v} пъти`]} />} />
                <Bar dataKey={brandsMode === "spend" ? "spend" : "count"} radius={[0, 4, 4, 0]} barSize={14}>
                  {topBrands.map((b) => (
