...
const wt   = (pairs) => faker.helpers.weightedArrayElement(pairs.map(([value, weight]) => ({ value, weight })));
...
const BG_BRANDS_BY_CATEGORY = {
  ...
  "Напитки":                    ["Загорка", "Каменица", "Астика", "Шуменско", "Бургаско", "Плевенско", "Девин", "Банкя", "Горна Баня", "Боровец", "Cappy", "Нескафе", "Jacobs", "Suhindol", "Brestnik"],
  "Основни храни и консерви":   ["Хармоника", "Флориол", "Злато", "Barilla", "Олимп", "Хармоника Био"],
...
const brand = (category) => faker.helpers.arrayElement(BG_BRANDS_BY_CATEGORY[category] ?? ["Хармоника"]);
const maybeNoBrand = (category) => faker.datatype.boolean({ probability: 0.15 }) ? null : brand(category);
...
const CATEGORIES = {
  "Млечни и яйца": {
    weight: 5, priceRange: [1.5, 8.5],
    factory(cat) {
      const type = pick("Прясно Мляко", "Кисело Мляко", "Сирене Бяло", "Кашкавал", "Масло", "Skyr", "Крем Сирене", "Яйца", "Извара");
      const suffix = type === "Яйца" ? g.count() : (type === "Прясно Мляко" || type === "Кисело Мляко") ? g.pct() + " " + g.volume() : g.weight();
      return { bg: `${type} ${brand(cat)} ${suffix}`, en: `${type} ${suffix}`, unit_type: 2 };
    },
  },

  "Плодове и зеленчуци": {
    weight: 5, priceRange: [1.2, 6.0],
    factory() {
      const produce = [
        ["Банани", 1], ["Ябълки", 1], ["Портокали", 1], ["Домати", 2],
        ["Краставици", 2], ["Моркови", 2], ["Картофи", 2], ["Лук", 2],
        ["Салата", 2], ["Броколи", 2], ["Спанак", 2], ["Лимони", 2],
        ["Тиквички", 2], ["Чесън", 2], ["Чушки", 2],
      ];
      ...
      return { bg: `${name} ${variant} ${suffix}`.replace(/\s+/g, " ").trim(), en: `${name} ${suffix}`, unit_type: unitType };
    },
  },

  "Напитки": {
    weight: 3, priceRange: [0.9, 20.0],
    factory(cat) {
      const type = pick("Газирана Вода", "Негазирана Вода", "Сок", "Бира", "Вино", "Кафе на Зърна", "Кафе Смляно", "Чай");
  ...
  "Основни храни и консерви": {
    weight: 4, priceRange: [1.2, 18.0],
    factory(cat) {
      const type = pick(
        "Паста", "Ориз", "Брашно", "Захар", "Олио", "Зехтин",
        "Доматена Паста", "Консерва Риба Тон", "Мед", "Оцет", "Соев Сос",
        "Бульон", "Конфитюр", "Фъстъчено Масло",
      );
      return { bg: `${type} ${brand(cat)} ${g.weight()}`, en: `${type} ${g.weight()}`, unit_type: 2 };
    },
  },

  "Сладко и солено": {
    weight: 3, priceRange: [1.5, 12.0],
    factory(cat) {
      const type = pick(
        "Шоколад", "Бисквити", "Чипс", "Ядки", "Мюсли",
        "Овесени Ядки", "Вафли", "Крекери", "Халва", "Локум",
      );
      return { bg: `${type} ${brand(cat)} ${g.weight()}`, en: `${type} ${g.weight()}`, unit_type: 2 };
    },
  },
  ...
  "Пекарна": {
    weight: 3, priceRange: [1.5, 6.0],
    factory(cat) {
      const type = pick("Хляб", "Хляб Пълнозърнест", "Питки", "Кроасани", "Кори за Баница", "Хамбургери", "Козунак");
  ...
  "Месо и риба": {
    weight: 3, priceRange: [5.0, 18.0],
    factory(cat) {
      const [type, unitType] = faker.helpers.arrayElement([
        ["Пилешки Гърди", 1], ["Пилешки Бутчета", 1],
        ["Кайма Смесена", 2], ["Кайма Телешка", 2],
        ["Кебапчета", 2], ["Кюфтета", 2],
        ["Сьомга Парче", 2], ["Риба Скумрия", 2],
        ["Свинско Контра Филе", 2],
      ]);
      const suffix = unitType === 1 ? "1кг" : g.weight();
      return { bg: `${type} ${brand(cat)} ${suffix}`, en: `${type} ${suffix}`, unit_type: unitType };
    },
  },

  "Колбаси и деликатеси": {
    weight: 2, priceRange: [3.5, 9.0],
    factory(cat) {
      const type = pick("Шунка Варена", "Шунка Печена", "Луканка", "Салам", "Наденица", "Пастърма");
      return { bg: `${type} ${brand(cat)} Слайс ${g.weight()}`, en: `${type} Slice ${g.weight()}`, unit_type: 2 };
    },
  },

  "За дома и офиса": {
    weight: 2, priceRange: [2.5, 22.0],
    factory(cat) {
      const type = pick(
        "Препарат Съдове", "Гел за Пране", "Омекотител",
        "Тоалетна Хартия", "Кухненска Хартия", "Торби за Смет",
        "Препарат Стъкла", "Препарат Тоалетна", "Антибактериален Спрей",
      );
  ...
  "Козметика и лична грижа": {
    weight: 2, priceRange: [3.0, 15.0],
    factory(cat) {
      const type = pick(
        "Шампоан", "Балсам за Коса", "Дезодорант", "Течен Сапун",
        "Паста за Зъби", "Душ Гел", "Крем за Лице", "Мицеларна Вода",
      );
      return { bg: `${type} ${brand(cat)} ${g.volume()}`, en: `${type} ${g.volume()}`, unit_type: 2 };
    },
  },

  "Замразени храни": {
    weight: 2, priceRange: [2.5, 11.0],
    factory(cat) {
      const type = pick(
        "Грах Замразен", "Царевица Замразена", "Микс Зеленчуци Замразен",
        "Картофи Замразени", "Пица", "Кюфтета Замразени",
        "Риба Панирана Замразена", "Спанак Замразен",
      );
      return { bg: `${type} ${brand(cat)} ${g.weight()}`, en: `${type} ${g.weight()}`, unit_type: 2 };
    },
  },
  ...
  "Био": {
    weight: 1, priceRange: [3.0, 16.0],
    factory(cat) {
      const type = pick(
        "Био Ленено Семе", "Био Чиа Семена", "Био Кокосово Масло",
        "Био Овесени Ядки", "Био Кафява Захар", "Био Бадемово Мляко",
        "Био Конопено Семе", "Био Киноа",
      );
      return { bg: `${type} ${brand(cat)} ${g.weight()}`, en: `${type} ${g.weight()}`, unit_type: 2 };
    },
  },
...
function weightedSample(arr, getWeight, n) {
  ...
  while (result.length < Math.min(n, pool.length)) {
    ...
    for (let i = 0; i < pool.length; i++) {
      r -= weights[i];
      if (r <= 0) { result.push(...pool.splice(i, 1)); break; }
    }
  }
  return result;
}
...
function buildItem(product, shippingDate) {
  ...
  const promoPrice = isPromo ? +( price * faker.number.float({ min: 0.75, max: 0.92 })).toFixed(2) : null;
  const actualPrice = promoPrice ?? price;

  const qty = product.unit_type === 1
...
function buildOrder(shippingDate) {
  ...
  return {
    ...
    grouped_items: Object.entries(byCategory).map(([group_name, group_items]) => ({ group_name, group_items })),
    additional_orders: [],
