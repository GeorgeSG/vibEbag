...
| Routing | React Router v7 | Five-page app (Overview, Products, Orders, Реалитест, Categorizer) |
| Data storage | SQLite (better-sqlite3) | Local database with WAL mode; all data is personal and gitignored |
...
On first launch, a login form will appear. You can authenticate in two ways:
- **Email & password** — enter your eBag credentials. The app logs in via Playwright (headless) and saves session cookies automatically.
- **Session cookie** — paste your `sessionid` cookie from the browser DevTools. Useful if you log in to eBag via Google, Apple, or Facebook.

After login, the app fetches all your orders and imports them into a local SQLite database. To re-fetch orders (e.g. after new deliveries), click the sync button (↻) in the header. If your session has expired, the app will automatically re-login using saved credentials, or prompt you to log in again if only a session cookie was used.

Alternatively, you can run the login and data fetching manually from the terminal:

```bash
cd server && npm run login    # authenticate and save session cookies
cd server && npm run fetch    # fetch all orders (incremental)
```
...
Runs the dashboard with synthetic data — no real credentials needed.
...
The dashboard will be available at `http://localhost:3000`. Data is persisted in the `./data` volume.
